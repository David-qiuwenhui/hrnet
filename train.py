...
from nets.hrnet_training import get_lr_scheduler, set_optimizer_lr, weights_init
...
# 如果格式有误，参考：https://github.com/bubbliiiing/segmentation-format-fix
    ...
    调参是一门蛮重要的学问，没有什么参数是一定好的，现有的参数是我测试过可以正常训练的参数，因此我会建议用现有的参数。
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="pytorch hrnet training")
    # ---------- dataset and model params ----------
    parser.add_argument(
        "--data-path", default="../../dataset/SUIMdevkit", type=str, help="dataset root"
    )
    parser.add_argument(
        "--backbone", default="hrnetv2_w32", help="hrnetv2_18, hrnetv2_w32, hrnet_w48"
    )
    ...
    parser.add_argument("--freeze-batch-size", default=2, type=int)
    parser.add_argument("--unfreeze-batch-size", default=2, type=int)
    parser.add_argument(
        "--freeze-epochs",
        default=0,
        type=int,
